<?php
    ...
    $categoryget = (int)$_GET['key'];
    $categoryone = R::findOne('categories', 'id = ?', [$categoryget]);

    if (isset($data['do_edit'])) {
        $categoryone = R::findOne('categories', 'id = ?', [$categoryget]);
        $categoryone->category_name = $data['category_name'];
        $categoryone->categoryind = $data['categoryind'];

        $productschange = (array)$categoryone->categoryind;

        $productscatid = R::findAll('products', 'categoryind = ?', [$productschange]);
        foreach ($productscatid as $productsid) {
            $productsid->categoryind = $data['categoryind'];
            R::store($productsid);
        }
        R::store($categoryone);
    ...
    if (isset($data['do_delete'])) {
        $delete = R::findOne('categories', 'id = ?', [$categoryget]);
        R::trash($delete);
        header('location: categories.php');
    }
?>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-6">
                        <h1 class="mt-4">Category view</h1>
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-table me-1"></i>
                            </div>
                            <div class="card-body">
                                <form action="categoriesview.php?key=<?php echo $categoryget ?>" method="POST">
                                    <div>
                                        <label>Category name</label>
                                        <input name="category_name" class="dataTable-input" type="text" value="<?php echo $categoryone->category_name ?>">
                                    </div>
