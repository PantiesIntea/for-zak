<?php
    ...
    $characteristicget = (int)$_GET['key'];
    $characteristicsone = R::findOne('characteristics', 'id = ?', [$characteristicget]);

    $fcat = $characteristicsone->category;
    $fcategory  = R::findOne( 'categories', 'id = ?', ["$fcat"]); 

    if (isset($data['do_edit'])) {
        $characteristicsone = R::findOne('characteristics', 'id = ?', [$characteristicget]);
        $characteristicsone->characteristic_name = $data['characteristic_name'];

        $cat = $data['cat_id'];

        $category  = R::findOne( 'categories', ' category_name = ? ', ["$cat"]);
        $cat_id = $category->id;
        $characteristicsone->category = $category->id;

        R::store($characteristicsone);
        header('location: characteristics.php');
    }

    if (isset($data['do_delete'])) {
        $delete = R::findOne('characteristics', 'id = ?', [$characteristicget]);
        R::trash($delete);
        header('location: characteristics.php');
    }
?>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-6">
                        <h1 class="mt-4">Characteristic view</h1>
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-table me-1"></i>
                            </div>
                            <div class="card-body">
                                <form action="characteristicsview.php?key=<?php echo $characteristicget ?>" method="POST">
                                    <div>
                                        <label>Name</label>
                                        <input name="characteristic_name" class="dataTable-input" type="text" value="<?php echo $characteristicsone->characteristic_name ?>">
                                    </div>
