<?php
    ...
    if(isset($data['do_addnewproductclass'])) {
        $errors = array();
        if(trim($data['productclass_name']) == '' ) {
            $errors[] = 'Введите имя!';
        }

        if (empty($errors)) {
        $productclass = R::dispense('productclass');
        $productclass->productclass_name = $data['productclass_name'];
        $productclass->category = $data['cat_id'];

        $cat = $data['cat_id'];

        $category  = R::findOne( 'categories', ' category_name = ? ', ["$cat"]);
        $cat_id = $category->id;
        $productclass->category = $category->id;

        R::store($productclass);
                ...
                <main>
                    <div class="container-fluid px-6">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <form action="./addnewproductclass.php" method="POST">
                                    <div>
                                        <label>Name</label>
                                        <input name="productclass_name" class="dataTable-input" type="text">
                                    </div>
                                    <div>
                                    ...
                                    <button name="do_addnewproductclass" style="margin-top: 10px;" class="btn btn-primary">Create</button>
                                </form>
                            </div>
                        </div>
                        </div>
