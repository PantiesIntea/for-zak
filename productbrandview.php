<?php
    ...
    $productbrandget = (int)$_GET['key'];
    $productbrandone = R::findOne('productbrand', 'id = ?', [$productbrandget]);

    if (isset($data['do_edit'])) {
        $productbrandone = R::findOne('productbrand', 'id = ?', [$productbrandget]);
        $productbrandone->productbrand_name = $data['productbrand_name'];

        R::store($productbrandone);
        header('location: productsbrand.php');
    }

    if (isset($data['do_delete'])) {
        $delete = R::findOne('productbrand', 'id = ?', [$productbrandget]);
        R::trash($delete);
        header('location: productsbrand.php');
    }
?>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-6">
                        <h1 class="mt-4">Product brand view</h1>
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-table me-1"></i>
                            </div>
                            <div class="card-body">
                                <form action="productbrandview.php?key=<?php echo $productbrandget ?>" method="POST">
                                    <div>
                                        <label>Product class name</label>
                                        <input name="productbrand_name" class="dataTable-input" type="text" value="<?php echo $productbrandone->productbrand_name ?>">
                                    </div>
