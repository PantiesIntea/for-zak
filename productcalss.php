            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Product calss</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Product calss</li>
                        </ol>
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-table me-1"></i>
                                Product calss <a href="addnewproductclass.php" style="float: right;">Add new</a>
                            </div>
                            <div class="card-body">
                                <table id="datatablesSimple">
                                    ...
                                    <tbody>
                                            <?php
                                                $url = 'productclassview.php?key=';
                                                $productsclass = R::findAll('productclass');
                                                foreach ($productsclass as $productclass) {
                                                $class_num = $productclass->id;
                                                $cat = $productclass->category;
                                                $category  = R::findOne( 'categories', 'id = ?', ["$cat"]);  
                                                echo '<tr><td>'. $productclass->id. '</td>'.
                                                '<td>'. $productclass->productclass_name.' ('.
                                                $count = R::count('products', 'productclass = ?', ["$class_num"]).
                                                ')</td>'.
                                                '<td>'. $category->category_name. '</td>'.
                                                '<td><a href="'.$url.$productclass->id.'">view</a>'.'</td></tr>';
                                                }
                                            ?>
                                    </tbody>
                                </table>
