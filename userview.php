<?php
    ...
    $userget = (int)$_GET['key'];
    $userone = R::findOne('users', 'id = ?', [$userget]);

    if (isset($data['do_edit'])) {
        $userone = R::findOne('users', 'id = ?', [$userget]);
        $userone->email = $data['email'];
        R::store($userone);
        header('location: /');
    }

    if (isset($data['do_delete'])) {
        $delete = R::findOne('users', 'id = ?', [$userget]);
        R::trash($delete);
        header('location: /');
    }
?>
                ...
                <main>
                    <div class="container-fluid px-6">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <form action="userview.php?key=<?php echo $userget ?>" method="POST">
                                    <div>
                                        <label>Email</label>
                                        <input name="email" class="dataTable-input" type="email" value="<?php echo $userone->email ?>">
                                    </div>
