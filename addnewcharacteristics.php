<?php
    ...
    if(isset($data['do_addnewcharacteristic'])) {
        $errors = array();
        if(trim($data['characteristic_name']) == '' ) {
            $errors[] = 'Введите имя!';
        }

        if (empty($errors)) {
        $characteristics = R::dispense('characteristics');
        $characteristics->characteristic_name = $data['characteristic_name'];

        $cat = $data['cat_id'];

        $category  = R::findOne( 'categories', ' category_name = ? ', ["$cat"]);
        $cat_id = $category->id;
        $characteristics->category = $category->id;

        R::store($characteristics);
                ...
                <main>
                    <div class="container-fluid px-6">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <form action="./addnewcharacteristics.php" method="POST">
                                    <div>
                                        <label>Name</label>
                                        </div>
                                        <input name="characteristic_name" class="dataTable-input" type="text">
                                        <div>
                                    ...
                                    <button name="do_addnewcharacteristic" style="margin-top: 10px;" class="btn btn-primary">Create</button>
                                </form>
                            </div>
                        </div>
                        </div>
