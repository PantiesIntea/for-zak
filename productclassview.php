<?php
    ...
    $productclassget = (int)$_GET['key'];
    $productclassone = R::findOne('productclass', 'id = ?', [$productclassget]);

    if (isset($data['do_edit'])) {
        $productclassone = R::findOne('productclass', 'id = ?', [$productclassget]);
        $productclassone->productclass_name = $data['productclass_name'];

        $cat = $data['cat_id'];

        $category  = R::findOne( 'categories', ' category_name = ? ', ["$cat"]);
        $cat_id = $category->id;
        $productclassone->category = $category->id;

        R::store($productclassone);
        header('location: productcalss.php');
    }

    if (isset($data['do_delete'])) {
        $delete = R::findOne('productclass', 'id = ?', [$productclassget]);
        R::trash($delete);
        header('location: productcalss.php');
    }
?>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-6">
                        <h1 class="mt-4">Product class view</h1>
                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-table me-1"></i>
                            </div>
                            <div class="card-body">
                                <form action="productclassview.php?key=<?php echo $productclassget ?>" method="POST">
                                    <div>
                                        <label>Product class name</label>
                                        <input name="productclass_name" class="dataTable-input" type="text" value="<?php echo $productclassone->productclass_name ?>">
                                    </div>
                                    <div>
                                    ...
                                    <select name="cat_id" class="dataTable-input" type="text">
                                        ...
                                        $cat = $productclassone->category;
                                        $category  = R::findOne( 'categories', 'id = ?', ["$cat"]);  
                                        ?>
                                        <option value="<?php echo $category->category_name ?>" selected hidden><?php echo $category->category_name ?></option>
                                        </select>
                                    </div>
